# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
import grpc

import cloudframe.protos.function_pb2 as function__pb2


class GreeterStub(object):
  # missing associated documentation comment in .proto file
  pass

  def __init__(self, channel):
    """Constructor.

    Args:
      channel: A grpc.Channel.
    """
    self.Call = channel.unary_unary(
        '/FunctionData.Greeter/Call',
        request_serializer=function__pb2.FunctionRequest.SerializeToString,
        response_deserializer=function__pb2.FunctionReply.FromString,
        )


class GreeterServicer(object):
  # missing associated documentation comment in .proto file
  pass

  def Call(self, request, context):
    # missing associated documentation comment in .proto file
    pass
    context.set_code(grpc.StatusCode.UNIMPLEMENTED)
    context.set_details('Method not implemented!')
    raise NotImplementedError('Method not implemented!')


def add_GreeterServicer_to_server(servicer, server):
  rpc_method_handlers = {
      'Call': grpc.unary_unary_rpc_method_handler(
          servicer.Call,
          request_deserializer=function__pb2.FunctionRequest.FromString,
          response_serializer=function__pb2.FunctionReply.SerializeToString,
      ),
  }
  generic_handler = grpc.method_handlers_generic_handler(
      'FunctionData.Greeter', rpc_method_handlers)
  server.add_generic_rpc_handlers((generic_handler,))
